...
var overtureLinks = function() {
  ...
  this.setItems = function(listItems) { this.listItems = listItems; }
...
function createOvertureSearchLink(conf) {
   ...
   if ( !isEmpty(keywordCharEnc) ) {
      retString += "&keywordCharEnc=" + encodeURI(keywordCharEnc);
   }

   if ( !isEmpty(outputCharEnc) ) {
      retString += "&outputCharEnc=" + encodeURI(outputCharEnc);
   }

   if ( !isEmpty(keywords) ) {
      retString += "&Keywords=" + encodeURIComponent(keywords);
   } else {
	   retString += "&Keywords=";
	}
   ...
   if (!isEmpty(serveUrl)) {
	   retString += "&serveUrl=" + encodeURIComponent(serveUrl);
   }
...
//Content Match links
function loadOvertureCMInclude(conf, doPrint) {
  var url = createOvertureCMLink(conf);
  if(doPrint) document.writeln("<br />Overture ContentMatch url: " + url + "<br />");
  printSrcInclude(url);
}

//Creates and prints the javascript src= code to include the 
//Search links
function loadOvertureSearchInclude(conf, doPrint) {
  var url = createOvertureSearchLink(conf);
  if(doPrint) document.writeln("<br />Overture Search url: " + url + "<br />");
  printSrcInclude(url)
}

//Creates and prints the javascript src= code to include the 
//Links spots
function loadOvertureLinkspotInclude(conf, doPrint) {
  var url = createOvertureLinkspotLink(conf);
  if(doPrint) document.writeln("<br />Overture LinkSpot url: " + url + "<br />");
  printSrcInclude(url)
}
